            ...
            <form action="{{ route('admin.invoices.update', $invoice) }}" method="POST" id="invoiceForm">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    ...
                    <div>
                        ...
                        <input type="number" name="vat_rate" id="vat_rate"
                               ...
                               value="{{ old('vat_rate', $invoice->vat_rate) }}" step="0.01" min="0" max="100" required>
                        @error('vat_rate')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="mb-6">
                    ...
                    <textarea name="notes" id="notes" rows="3"
                              class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 @error('notes') border-red-500 @enderror">{{ old('notes', $invoice->notes) }}</textarea>
                    @error('notes')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-6">
                    ...
                    <textarea name="terms_conditions" id="terms_conditions" rows="3"
                              class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 @error('terms_conditions') border-red-500 @enderror">{{ old('terms_conditions', $invoice->terms_conditions) }}</textarea>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-12 gap-4 item-row">
                            <div class="md:col-span-5">
                                ...
                                <input type="text" name="items[{{ $index }}][description]"
                                       class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 item-description"
                                       value="{{ $item->description }}" required>
                            </div>
                            <div class="md:col-span-2">
                                <label class="block text-sm font-medium text-gray-700 mb-2">Quantity *</label>
                                <input type="number" name="items[{{ $index }}][quantity]"
                                       class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 item-quantity"
                                       value="{{ $item->quantity }}" min="1" required>
                            </div>
                            <div class="md:col-span-2">
                                <label class="block text-sm font-medium text-gray-700 mb-2">Unit Price *</label>
                                <input type="number" name="items[{{ $index }}][unit_price]"
                                       class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 item-unit-price"
                                       value="{{ $item->unit_price }}" step="0.01" min="0" required>
                            </div>
                            <div class="md:col-span-2">
                                <label class="block text-sm font-medium text-gray-700 mb-2">Total</label>
                                <input type="text" class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm bg-gray-50 item-total"
                    ...
                    <div class="flex justify-end">
                        <div class="w-64">
                            <div class="flex justify-between py-2">
                                ...
                                <span class="text-gray-900" id="subtotal">{{ number_format($invoice->subtotal, 2) }}</span>
                            </div>
                            <div class="flex justify-between py-2">
                                <span class="font-medium text-gray-700">VAT ({{ $invoice->vat_rate }}%):</span>
                                <span class="text-gray-900" id="vat-amount">{{ number_format($invoice->vat_amount, 2) }}</span>
                            </div>
                            <div class="flex justify-between py-2 border-t border-gray-300">
                                <span class="font-bold text-gray-900">Total:</span>
                                <span class="font-bold text-gray-900" id="total-amount">{{ number_format($invoice->total_amount, 2) }}</span>
                            </div>
                        </div>
                    </div>
                </div>
